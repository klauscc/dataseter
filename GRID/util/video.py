...
#   description: copied from https://github.com/rizkiarm/LipNet/blob/master/lipnet/lipreading/videos.py
#
#================================================================
...
class VideoAugmenter(object):
    ...
    def split_words(video, align):
        ...
        for sub in align.align:
            ...
            # Create new align
            _align = Align(align.absolute_max_string_len,
                           align.label_func).from_array([(0, sub[1] - sub[0],
                                                          sub[2])])
            # Append
    ...
    def merge(video_aligns):
        ...
        video.face = np.ones(
            (0, vsample.face.shape[1], vsample.face.shape[2],
             vsample.face.shape[3]),
            dtype=np.uint8)
        video.mouth = np.ones(
            (0, vsample.mouth.shape[1], vsample.mouth.shape[2],
             vsample.mouth.shape[3]),
            dtype=np.uint8)
        align = []
        inc = 0
        ...
        align = Align(asample.absolute_max_string_len,
                      asample.label_func).from_array(align)
        return (video, align)
    ...
    def pick_word(video, align):
        ...
        return video_aligns[np.random.randint(video_aligns.shape[0],
                                              size=2), :][0]
    ...
    def pad(video, length):
        ...
        face_padding = np.ones(
            (pad_length, video.face.shape[1], video.face.shape[2],
             video.face.shape[3]),
            dtype=np.uint8) * 0
        mouth_padding = np.ones(
            (pad_length, video.mouth.shape[1], video.mouth.shape[2],
             video.mouth.shape[3]),
            dtype=np.uint8) * 0
        _video = Video(video.vtype, video.face_predictor_path)
        _video.face = np.concatenate(
            (video.face[0:video_length], face_padding), 0)
        _video.mouth = np.concatenate(
...
class Video(object):
    ...
    def from_frames(self, path):
        ...
        frames = [sio.imread(frame_path) for frame_path in frames_path]
    ...
    def get_frames_mouth(self, detector, predictor, frames):
        ...
        for frame in frames:
            ...
            if shape is None:  # Detector doesn't detect face, just return as is
                print('    detector does not detect face.')
                return frames
            mouth_points = []
    ...
    def set_data(self, frames):
        ...
        for frame in frames:
            frame = frame.swapaxes(
                0, 1)  # swap width and height to form format W x H x C
            if len(frame.shape) < 3:
                frame = np.array([frame]).swapaxes(0, 2).swapaxes(
                    0, 1)  # Add grayscale channel
